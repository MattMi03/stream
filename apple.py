...
class MultiHttpStreamerApp:
    ...
    def add_camera_with_url(self, url):
        ...
        # 预览标签：填充容器，初始无图片时显示文字，图像自适应
        preview_label = ctk.CTkLabel(preview_frame, text=f"摄像头 {cam_id+1} 预览",
                                     width=0, height=0,  # 设为0，让 pack 决定大小
                                     corner_radius=6)
        ...
        while cam_data['running']:
            try:
                ...
                while cam_data['running'] and cap.isOpened():
                    ...
                    # 更新标签，清除文字
                    def update_label(lbl, img_obj):
                        lbl.configure(image=None)
                        lbl.configure(image=img_obj, text="")

                    self.root.after(0, update_label, label, imgtk)
